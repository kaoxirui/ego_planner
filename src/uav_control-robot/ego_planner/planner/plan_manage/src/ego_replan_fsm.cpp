...
namespace ego_planner
{
  ...
  void EGOReplanFSM::init(ros::NodeHandle &nh)
  {
    ...
    {
      waypoint_sub_ = nh.subscribe("/local_planner/goal_position", 1, &EGOReplanFSM::waypointCallback, this);
//      waypoint_sub_ = nh.subscribe("/move_base_simple/goal", 1, &EGOReplanFSM::waypointCallback, this);
  ...
  { 
    cout << "Got setpoint for fcu!" << endl;

    if (msg.pose.position.z < -0.1)
      return;
    if(fabs(msg.pose.position.x )>100 || fabs(msg.pose.position.y )>100)
          return;
    cout << "Triggered!" << endl;
    // trigger_ = true;
    init_pt_ = odom_pos_;
